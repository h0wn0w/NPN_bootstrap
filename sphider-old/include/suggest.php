...
//  *****   enter here for media suggestions
        if ($media_only =='1') {
            ...
            if ($db2_slv == 1) {
                ...
                $media_results = $media_results + $media_results2;
            }
            ...
            if ($db3_slv == 1) {
                ...
                $media_results = $media_results + $media_results3;
            }
            ...
            if ($db4_slv == 1) {
                ...
                $values = $media_results + $media_results4;
            }
            ...
            if ($db5_slv == 1) {
                ...
                $media_results = $media_results + $media_results5;
            }
...
//   ********       enter here for text  suggestions
            ...
            if ($db5_slv == 1) {
                ...
                $values = $values + $values5;
            }

            if ($min_sug_chars != '0' && is_array($values)) {   // if we do have results, loop through them and add them to the output . Max. = $suggest_rows

                arsort($values);
                $val_array  = array_slice($values, 0, $suggest_rows, TRUE);
                $sug_array  = array_keys($val_array);  //  we just need the suggestions, without weighting

                if ($sugg_not) {        //  Obey the list of words not to be suggested
                    $suggest_not = file($common_dir.'suggest_not.txt'); //  get all words not to be suggested

                    foreach ($sug_array as $this_sug){
                        $met = 0;
                        foreach ($suggest_not as $invalid) {
                            if (preg_match("/\S/", $invalid)) {         //  remove blank rows
                                $invalid = trim($invalid);              //  remove blanks from forbidden words
                            }

                            if (preg_match("/$invalid/si", $this_sug)) {    //  compare suggestion with all forbidden words
                                $met = 1;
                            }
                        }
                        if (!$met) {    //  if not met any of the forbidden words, add new suggestion
                            $sug_array1[] = $this_sug;
                        }
                    }
                    $sug_array = $sug_array1;
                }

                // Depending on PHP version, the function json_encode()  is unable to transfer pure numbers,
                //  see:     http://www.php.net/manual/en/function.json-encode.php#86513
                //  we need to delude this function. Thus, suggestions like "2014" will be transferred
                $i = 0;
                foreach ($sug_array as $this_sug){
                    $this_sug = " $this_sug ";
                    if(preg_match("@ (\d+) @", $this_sug)) {
                        $this_sug = str_replace(" ", "\"", $this_sug);  //  add additional double quotes in front and behind
                        $sug_array[$i] = '';
                        $sug_array[$i] = $this_sug;
                        $i++;
                    }
                }
    ...
    //  save all in tmp folder as 'suggest.txt'
    //$suggest_file   = "./tmp/suggest.txt";   
    $content = '';
    $suggest_file   = "./tmp/suggest.txt";    
    $content  = "$keyword\r\n";
    $content .= "$media_only\r\n";
    ...
    $content .= "$db\r\n";
    $content .= "$prefix\r\n";
    file_put_contents($suggest_file, $content); 
/*
    $content = '';
    $suggest_file   = "./tmp_1/suggest.txt";    
    $content  = "$keyword\r\n";
    $content .= "$media_only\r\n";
    $content .= "$type\r\n";
    $content .= "$catid\r\n";
