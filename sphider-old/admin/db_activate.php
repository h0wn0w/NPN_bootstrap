<?php
    ...
    if (isset($submit))
    $submit = cleaninput(substr(trim($submit),0,25));

    if (isset($dba_act))
    $dba_act = substr(trim($dba_act),0,1);
    if (isset($dbu_act))
    $dbu_act = substr(trim($dbu_act),0,1);
    if (isset($dbs_act))
    $dbs_act = substr(trim($dbs_act),0,1);

    if (isset($_db1_slv))
    $_db1_slv = substr(trim($_db1_slv),0,1);
    if (isset($_db2_slv))
    $_db2_slv = substr(trim($_db2_slv),0,1);
    if (isset($_db3_slv))
    $_db3_slv = substr(trim($_db3_slv),0,1);
    if (isset($_db4_slv))
    $_db4_slv = substr(trim($_db4_slv),0,1);
    if (isset($_db5_slv))
    $_db5_slv = substr(trim($_db5_slv),0,1);

    if (isset($prefix1))
    $prefix1 = substr(trim($prefix1),0,255);
    if (isset($prefix2))
    $prefix2 = substr(trim($prefix2),0,255);
    if (isset($prefix3))
    $prefix3 = substr(trim($prefix3),0,255);
    if (isset($prefix4))
    $prefix4 = substr(trim($prefix4),0,255);
    if (isset($prefix5))
    $prefix5 = substr(trim($prefix5),0,255);
