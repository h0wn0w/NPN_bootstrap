<?php
    ...
    function ubbtexthandler($text, $ubbObj = null)
    {
        ...
        if(is_object($ubbObj)) if(strpos(strtolower($text), '/me') > 0) $text = preg_replace("/([^[])/me([^\n\r$]*)([\n\r$])/i", "\\1<span class=\"me\">*".$ubbObj->username." \\2 *</span>\\3", $text);
        $text = nl2br($text);
    ...
    class ubbParser
    {
        ...
        {
            $this->username = preg_replace('/([^a-z0-9_~]*)/i', '', $username);
        }
