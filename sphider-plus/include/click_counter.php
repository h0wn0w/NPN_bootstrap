...
/******************************************************
 This script updates the columns click_counter and the column 'last_click' in table 'links'
 ...
 *******************************************************/
    ...
    if ($result->num_rows) {
        ...
        if ($last_click+$click_wait < $time) {      //  prevent  promoted clicks, else remember this click
            $sql_query = "UPDATE ".$mysql_table_prefix."links set click_counter=click_counter+1, last_click='$time', last_query='$query', ip='$client_ip' where url = '$url' LIMIT 1";
    ...
    exit ('');      //  Good-bye, we've got your page click.
