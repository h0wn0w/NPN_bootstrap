<?php
    ...
    function search($searchstr, $category, $start, $per_page, $type, $domain, $prefix) {
        ...
        //  limit number of results per page to 1 ( ignore arguments behind the ?  in URL)
        if ($sort_results == '8'){
            $page_res   = array();
            $known_page = array();

            foreach($res as &$v) {
                $page = substr($v['url'], 0, strpos($v['url'], "?"));   //  fetch actual page from result array
                if (!$page) {
                    $page = $v['url'];
                }

                if (!in_array($page, $known_page)) {    //  must be a new URL
                    $known_page[]   = $page;
                    $page_res[]     = $v;
                }
            }
...
//echo "\r\n\r\n<br>res Array1:<br><pre>";print_r($res);echo "</pre>\r\n";
    ...
    function slave_search($searchstr, $category, $domain, $mysql_table_prefix, $start, $per_page, $db_slv, $type, $yet_results) {
        ...
        $notlist            = array();
        $domains            = array();
//echo "\r\n\r\n<br>searchstr in slave_search():<br><pre>";print_r($searchstr);echo "</pre>\r\n";
        ...
        while ($not_words < count($wordarray)) {
            ...
            $sql_query1 = "SELECT link_id from ".$mysql_table_prefix."link_keyword$wordmd5, ".$mysql_table_prefix."keywords where ".$mysql_table_prefix."link_keyword$wordmd5.keyword_id= ".$mysql_table_prefix."keywords.keyword_id and keyword='$searchword'";
...
//    ****        for 'STRICT search' enter here
        if ($strictpos === 0) {
            ...
            if (!$num_rows) {   // if there was no searchword in table keywords
                ...
                if ($sort_results == '7') {   //      get query hit results
                    $sql_query1 = "SELECT distinct * from ".$mysql_table_prefix."link_keyword$wordmd5, ".$mysql_table_prefix."keywords where ".$mysql_table_prefix."link_keyword$wordmd5.keyword_id= ".$mysql_table_prefix."keywords.keyword_id and keyword='$searchword' $domain_qry order by hits desc";
                } else {                    // get weight results
                    $sql_query1 = "SELECT * from ".$mysql_table_prefix."link_keyword$wordmd5  where keyword_id = '$keyword_id' order by weight desc";
                }
        ...
        //echo "\r\n\r\n<br>linklist Array:<br><pre>";print_r($linklist);echo "</pre>\r\n";
                ...
                if (!$num_rows) {   // if there was no searchword in table keywords
                    ...
                    $break = 1;
                     ...
                     while ($this_array = $result->fetch_array(MYSQLI_ASSOC)) {
                        ...
                        if ($sort_results == '7') {   //      get query hit results
                            $sql_query1 = "SELECT * from ".$mysql_table_prefix."link_keyword$wordmd5  where keyword_id = '$keyword_id' order by hits desc";
                        } else {                    // get weight results
                            $sql_query1 = "SELECT * from ".$mysql_table_prefix."link_keyword$wordmd5  where keyword_id = '$keyword_id' order by weight desc";
                        }
                        $result     = $db_con->query($sql_query);
...
//      *******  finally 'STANDARD search'
                    ...
                    if ($translit_el) { //  eventually we need to add links that are found after transliterating the query
                        ...
                        while (($words < count($wordarray)) && $possible_to_find == 1) {
                            ...
                            $wordmd5 = substr(md5($el_searchword), 0, 1);
                            if ($sort_results == '7') {   //      get query hit results
                                $sql_query1 = "SELECT distinct * from ".$mysql_table_prefix."link_keyword$wordmd5, ".$mysql_table_prefix."keywords where ".$mysql_table_prefix."link_keyword$wordmd5.keyword_id= ".$mysql_table_prefix."keywords.keyword_id and keyword='$el_searchword' $domain_qry order by hits desc";
                            } else {        // get weight results
                                $sql_query1 = "SELECT distinct * from ".$mysql_table_prefix."link_keyword$wordmd5, ".$mysql_table_prefix."keywords where ".$mysql_table_prefix."link_keyword$wordmd5.keyword_id= ".$mysql_table_prefix."keywords.keyword_id and keyword='$el_searchword' $domain_qry order by weight desc";
                            }
...
//  ***** end  different search modes
        ...
        //echo "\r\n\r\n<br>result_array_full10:<br><pre>";print_r($result_array_full);echo "</pre>\r\n";
        ...
        //echo "\r\n\r\n<br>result_array_full:<br><pre>";print_r($result_array_full);echo "</pre>\r\n";
        if ($clear == 1) {
            $temp_array = array();
            $linklist   = array();
        }
       ...
       if ($result_array_full) {
        ...
        //echo "\r\n\r\n<br>resul_array_temp Array:<br><pre>";print_r($result_array_temp);echo "</pre>\r\n";
        ...
        //echo "\r\n\r\n<br>result_array:<br><pre>";print_r($result_array);echo "</pre>\r\n";
        if ($count != '0') {
        ...
        while ($row = $result->fetch_array(MYSQLI_ASSOC)) {
            ...
            $res[$i]['description']     = stripslashes($row["description"]);
            $res[$i]['url']             = $row["url"];
...
//echo "\r\n\r\n<br>res array end of 'slave_search':<br><pre>";print_r($res);echo "</pre>";
    ...
    function get_text_results($query, $start, $category, $searchtype, $results, $domain, $loop, $orig_query, $prefix) {
        ...
        global $most_pop, $pop_rows, $tag_cloud, $color_cloud, $catid, $db, $add_url, $shot_dir;
        ...
        if ($handle = opendir($shot_dir)) {
            while (false != ($file = readdir($handle))) {
                if ($file != "." && $file != ".." && $file != "flood_file.txt" && $file != "suggest.txt") {
...
//echo "\r\n\r\n<br>result Array:<br><pre>";print_r($result);echo "</pre>\r\n";
        ...
        if ($rows>0) {  //  we got results and need to process them
            ...
            //  create ligatures and non-ligatures for highlighting the keywords
            if ($liga == "1") {
                $wild_orig = $all_wild;
                $wild_liga = '';
                $thiswords = array();
...
//echo "\r\n\r\n<br /> thisword10: '$thisword'<br />\r\n";
                    ...
                    if ($thisword1 == $thisword) {
                        ...
                        while ($char = each($phon_trans)) {
                            ...
                            if ($thisword1 != $thisword) {  //  break on first ligature
                                $thiswords['+'][] = html_entity_decode($thisword1, ENT_QUOTES, "UTF-8");
                                break;
                            }
                        }
                ...
                $words['+']   = array_unique(array_merge($words['+'], $thiswords['+']));
            }

            if ($strict_high) {
                $thiswords = array();
                foreach($words['+'] as $thishit) {
                    $thiswords['+'][] = " ".$thishit." ";
                }
                $words['+']   = $thiswords['+'];
            }
...
//echo "\r\n\r\n<br>result Array:<br><pre>";print_r($result);echo "</pre>\r\n";
            while ($i < $num_of_results && $i < $results_per_page) {
                ...
                $description    = " ".stripslashes($result[$i]['description']); //  remove backslashes
...
//echo "\r\n\r\n<br /> shot: '$shot'<br />\r\n";
                ...
                if(strlen($fulltxt) < 5) {
                    $fulltxt = $description;    //  in case there is no text
                    if(strlen($fulltxt) < 5) {
                        $fulltxt = $title;      //  in case there is no text
                    }
                }
                ...
                $words[]    = arsort($words['+']);    //  reverse order, to highlight voluminous words first
                if ($mb) {
                ...
                if ($txtlen > $desc_length && !$only_links) {
                    ...
                    if ($strictsearch) {     //      if STRICT search enter here
                        ...
                        $words['+'][0]  = $recovered;  //  replace without ' ! '
                        ...
                        foreach($words['+'] as $word) {
                            ...
                            while (!($found_in =='') && $hits < $max_hits && $found_in < $txtlen) {
                                ...
                                if ($found_in) {
                                    $places[] = $found_in;   //  remind absolute position of match
                                }
                                $hits++;
                            }
                        }
//echo "\r\n\r\n<br>places Array:<br><pre>";print_r($places);echo "</pre>\r\n";
                    ...
                    foreach($words['+'] as $word) {
                        if ($mb) {
                            if(!$tmp || $tmp == '   ') {
                                $tmp = $title;  //  as it seems, we try to find something in title
                            }
                ...
                $url    = reconvert_url($url);     //  recover blank characters
                $url2   = $url;
                ...
                if (!$only_links) {     //  not required, if search only search for link text. Already highlighted in function links_only()
                    ...
                    if ($places[0] == '1' && $show_warning == '1' && $type !='tol' && !$only_links || ( $show_warning == '1' && $weight == '0')) {  // if  no HTML text to highlight
                    ...
                    if($type == "phrase") {
                        $title      = highlight($title, $words['+s'][0], $highlight);
                        $fulltxt    = highlight($fulltxt, $words['+s'][0], $highlight);

                        if (!($strictpos === 0) && $index_host == '1' && !$only_links) {  //  not for strict search and link-only search
                            $url2 = highlight($url2, $words['+s'], $highlight);
                        }
                    } else {
                        foreach($words['+'] as $change) {
                            ...
                            //  if necessary, highlight the title
                            if($case_sensitive == '0') {
                                if($mb){
                                    if (mb_stripos($title, $change)){
                                        $title = highlight($title, $change, $highlight);
                                    }
                                } else {
                                    if (stripos($title, $change)){
                                        $title = highlight($title, $change, $highlight);
                                    }
                                }
                            } else {
                                if($mb){
                                    if (mb_strpos($title, $change)){
                                        $title = highlight($title, $change, $highlight);
                                    }
                                } else {
                                    if (strpos($title, $change)){
                                        $title = highlight($title, $change, $highlight);
                                    }
                                }
                            }

                            //  if necessary, highlight the URL
                            if (!($strictpos === 0) && $index_host == '1' && !$only_links) {  //  not for strict search and link-only search
                                if($mb){
                                    if (mb_stripos($url2, $change)){
                                        $url2 = highlight($url2, $change, $highlight);
                                    }
                                } else {
                                    if (stripos($url2, $change)){
                                        $url2 = highlight($url2, $change, $highlight);
                                    }
                                }
                            }
    ...
    function highlight($string, $change, $highlight) {
        ...
        if ($mb){
            ...
            if ($case_sensitive == 1 ) {
                ...
                while ($found_in !='' && $found_in < $txtlen) {         //  loop through all hits in full text
                    if ($change == "class") {
                        $string = str_replace("class=", "=ssalc", $string); //  replace 'class='
                    }
                    ...
                    if ($strict_high) {
                        $end = $found_in+$length+3;                     //   find end of word to be highlighted. +2 because< and > are added to $highlight
                    } else {
                        $end = mb_strpos($string, " ", $found_in+$length);   //   find end of word (for combined words) to be highlighted. +2 because< and > are added to $highlight
                    }
                ...
                if ($vowels || $noacc_el || $translit_el) {
                    ...
                    $found1  = '';
                    $found2  = '';
                    $found1  = mb_stripos($string_ex, $change);

                    if ($translit_el) {
                        $found2  = mb_stripos($string_ex, translit_el($change));
                        if (!$found1) $found1 = '999999999';
                        if (!$found2) $found2 = '999999999';
                        //  define  the minimum hit position
                        $found_in = min($found1, $found2);
                ...
                while ($found_in !='' && $found_in < $txtlen) {         //  loop through all hits in full text
                    ...
                    if ($strict_high) {
                        ...
                        $end = mb_strpos($string, " ", $found_in+$length);  //   find end of word (for combined words) to be highlighted.
                    }
                    $rest_all   = mb_substr($string, $end);
                    $string     = "".mb_substr($string, 0, $end)."</span> ".$rest_all."";    //  include highlight end-tag
                    $offset     = $end+7 ;  //  +7 because </span> was added
            ...
            $string = str_replace("=ssalc", "class=", $string); //  rebuild 'class='
            ...
            if ($case_sensitive == 1 ) {
                ...
                while ($found_in !='' && $found_in < $txtlen) {         //  loop through all hits in full text
                    ...
                    if ($strict_high) {
                        ...
                        $end = strpos($string, " ", $found_in+$length); //   find end of word (for combined words) to be highlighted.
                    }
            ...
            $string = str_replace("<span <span", "<span", $string); //  created by multiple hits per page
            $string = str_replace("=ssalc", "class=", $string);     //  rebuild 'class='
            $string = str_replace("  ", " ", $string);
            return $string;
        }
