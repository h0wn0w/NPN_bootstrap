<?php
    ...
    function image($query, $url, $media_type, $all, $urlx, $title1, $image_dir, $info, $info2, $thumb, $mode, $media_only, $type, $category, $catid, $mark, $db, $prefix, $domain) {
        global $include_dir, $admin_dir, $sph_messages, $mysql_table_prefix, $template, $template_dir, $index_id3, $limit_media, $delim, $inst_dir, $install_url;
        ...
        if ($media_results) {
            ...
            while (list($key, $value) = each($media_results)) {
                ...
                $link_crypt = str_replace("%25", "-__-", $link_crypt);  //  crypt the %25  because urldecode()  does not work
                $link_crypt = str_replace("%", "-___-", $link_crypt);   //  crypt the %  because urldecode()  does not work
                ...
                $thumb_link = str_replace("%25", "_", $thumb_link);
                $thumb_link = str_replace("%", "_", $thumb_link);
    ...
    function media($query, $url, $media_type, $all, $urlx, $title1, $image_dir, $info, $info2, $thumb, $mode, $media_only, $type, $category, $catid, $mark, $db, $prefix, $domain) {
        global $include_dir, $admin_dir, $sph_messages, $template, $template_dir, $index_id3, $limit_media, $delim;
        ...
        if ($media_results) {
            ...
            foreach($media_results as $value) {
                ...
                $playtime   = '';
                $time       = $id3_array[10];
                $playtime   = number_format(substr($time, strrpos($time, ';;')+3), 2, '.', '');  // get play time

                if ($playtime) {
                    if (stristr($time, "sec")) {
                        $duration = $sph_messages['seconds'];
                    } else {
                        $duration = $sph_messages['minutes'];
                    }

                    //make the playtime human readable
                    if ($playtime > 100 ) {
                        $playtime = $playtime/60;
                        $playtime = number_format($playtime, 2, '.', '');  // format the playtime
                ...
                $link_crypt = str_replace("&", "-_-", $value[3]);       //  crypt the & character
                $link_crypt = str_replace("%25", "-__-", $link_crypt);  //  crypt the %25  because urldecode()  does not work
                $link_crypt = str_replace("%", "-___-", $link_crypt);   //  crypt the %25  because urldecode()  does not work
                $link_click = "$include_dir/media_counter.php?url=$link_crypt&amp;query=$query&amp;db=$db&amp;prefix=$prefix&amp;client_ip=$client_ip";   //  redirect users click in order to update Most Media Popular Links
    ...
    function media_only($query, $start, $media_only, $type, $category, $catid, $mark, $db, $prefix, $domain) {
        ...
        global $dbu_act, $db1_slv, $db2_slv, $db3_slv, $db4_slv, $db5_slv, $elapsed, $inst_dir, $install_url;
        global $mytitle, $show_categories, $has_categories, $checked_cat, $tpl, $checked_all;
        global $adv, $advanced_search, $show_media, $description, $embedded, $up_advanced;
        global $out, $xml_dir, $xml_name, $vowels, $noacc_el, $translit_el, $delim, $viking, $smt_button;
    ...
    ";
        ...
        if ($media_count > '0') {
            ...
            for ($i=$from; $i<$to; $i++) {
                ...
                $media_crypt = str_replace("%25", "-__-", $media_crypt);    //  crypt the %25  because urldecode()  does not work
                $media_crypt = str_replace("%", "-___-", $media_crypt);     //  crypt the %  because urldecode()  does not work
                ...
                $thumb_link     = str_replace($inst_dir, $install_url, $thumb_link);
    ...
    function thislink_media($query, $link, $media_type, $all, $domain, $db_slv, $mysql_table_prefix) {
        ...
        $media_results  = array();
        $sort           = "title";
        $link           = $db_con-> real_escape_string($link);  //  SQL queries will be happy about this
       ...
       //  collect all results into one array
        ...
        foreach ($media_results as $this_media[$i]) {
            if ($this_media[$i][6] == 'image') {
                ...
                $file = str_replace("%25", "_", $file); //  should be   %   not     _
                $file = str_replace("%", "_", $file);   //  should be   %   not     _
    ...
    function fresh_media($query, $domain_qry, $mysql_table_prefix, $catid, $db_slv){
        global $db_con, $case_sensitive, $debug, $category, $search_id3;
