...
/*
     ...
     * @since       Version 0.1
     *
     * Modified by R. Kellner [Tec] 2013-03-31
     * In order to meet the Sphider-plus requirements:
     * - return all headers
     * - added info to header
     + - follow redirections with respect to calling script
     *
     */
...
//echo "\r\n\r\n<br>contentArray0:<br><pre>";print_r($content);echo "</pre>\r\n";
                $contentArray[0] = substr($content, 0, strpos($content, "\r\n\r\n"));
                $contentArray[1] = substr($content, strpos($content, "\r\n\r\n"));

                //$contentArray = explode("\r\n\r\n", $content);
//echo "\r\n\r\n<br>contentArray1:<br><pre>";print_r($contentArray);echo "</pre>\r\n";

                if (!$contentArray[0])
                {
                    $this->_setError('NOHOST');
                    curl_close($ch);
                    return FALSE;
                }
...
//echo "\r\n\r\n<br>CURL status Array:<br><pre>";print_r($status);echo "</pre>\r\n";
                $answer = substr($contentArray[0], 0, strpos($contentArray[0], "\r\n"));    //  offers the first row of first header
//echo "\r\n\r\n<br /> answer row0: '$answer'<br />\r\n";
                // Store the content
                $count = count($contentArray);
//echo "\r\n\r\n<br /> count: '$count'<br />\r\n";

                //$this->result = $contentArray[count($contentArray)];
                $this->result = $contentArray[count($contentArray) - 1];

                // Parse the headers
                //$this->_parseHeaders($contentArray[count($contentArray) - 2]);        //  this would fetch the final header, for automatically processing redirections
                $this->_parseHeaders($contentArray[0]);     //   Sphider-plus likes to fetch the first header, so we know eventually redirections
...
//echo "\r\n\r\n<br /> responseHeader: '$responseHeader'<br />\r\n";
                $status = $this->_parseHeaders($responseHeader);
//echo "\r\n\r\n<br /> status: '$status'<br />\r\n";
                // Do we have a 302 redirect ?
                if ($this->status == '302' && $this->redirect == TRUE)
                {
                    if ($this->curRedirect < $this->maxRedirect)        //  ERROR $curRedirect is undefined for first loop
                    {                                                   //  okay, because we want to know any redirection
