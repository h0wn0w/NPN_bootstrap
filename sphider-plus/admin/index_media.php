                        ...
                        if ($suffix =="php") {  //  use php file as command to download the image
                            ...
                            if (!$fp) {
                                ...
                                if ($linkstate != "Unreachable") {
                                    while ($answer) {       //  find name and suffix of the image to be downloaded
...
//$my_name = str_replace("%25", "_", $my_name);
//echo "\r\n\r\n<br /> my_name: $my_name<br />\r\n";
//echo "\r\n\r\n<br /> suffix: $suffix<br />\r\n";
                        //      try to find already indexed image with the same md5sum
                        $sql_query = "SELECT md5sum from ".$mysql_table_prefix."media where md5sum = '$new_md5'";
                        $result = $db_con->query($sql_query);
                        if ($debug && $db_con->errno) {
                            $err_row = __LINE__-2;
                            printf("<p><span class='red'>&nbsp;MySQL failure: %s&nbsp;\n<br /></span></p>", $db_con->error);
                        ...
                        if ($result->num_rows && $new_md5) {   //      enter only, if PHP function md5_file()  is allowed for remote files
                            if ($dup_media == '1') {    //  if we should index duplicate media on different pages
                                ...
                                $sql_query = "SELECT * from ".$mysql_table_prefix."media where md5sum = '$new_md5'";
                                $result = $db_con->query($sql_query);
                                if ($debug && $db_con->errno) {
                                    $err_row = __LINE__-2;
                                    printf("<p><span class='red'>&nbsp;MySQL failure: %s&nbsp;\n<br /></span></p>", $db_con->error);
...
//echo "\r\n\r\n<br /> localtempfile0: $localtempfile<br />\r\n";
...
//echo "\r\n\r\n<br /> localtempfile1: $localtempfile<br />\r\n";
                                mysqltest();
...
//$name = str_replace("%25", "_", $name);
//$title = str_replace("%25", "_", $title);
...
//echo "\r\n\r\n<br /> title1: $title<br />\r\n";
                                    ...
                                    $title_all  = $title;                                               //  add file name to title, so both become searchable
                                    $title_all .= " +-+ ".str_replace($trash1, $replace1, $name)."";    //  add also delimiter ' +-+ ' between title and file name

                                    //      store all information about actual image
                                    $sql_url        = $db_con->real_escape_string($url);
                                    $sql_link       = $db_con->real_escape_string($link);
                                    $sql_title      = $db_con->real_escape_string($title_all);
                                    $sql_id3_string = $db_con->real_escape_string($id3_string);
                                    $sql_my_name    = $db_con->real_escape_string($my_name);
                                    ...
                                    if (!$gd) {
                                        ...
                                        mysqltest();
//echo "\r\n\r\n<br /> im: '$im'<br />\r\n";
                                        if (!$im) { //  eventually the .jpg might be a .png. Try out
                                            $im = @imagecreatefrompng($localtempfile);
                                            if ($im) {  //  if the image is really a .png
                                                $suffix = "png";
                                            }
                                        }
                                        ...
                                        if (!$im) {
                                            ...
                                            //  work around, as urldecode() does not work here
                                            $thumb_file = str_replace("%25", "_", $thumb_file); //  should be   %   not     _
                                            $thumb_file = str_replace("%", "_", $thumb_file);   //  should be   %   not     _
                                            ...
                                            $sql_query = "UPDATE ".$mysql_table_prefix."media set thumbnail='$thumb', suffix='$suffix' where media_link like '$sql_link' ";
                                            $db_con->query ($sql_query);
                                            if ($debug && $db_con->errno) {
                                                $err_row = __LINE__-2;
                                                printf("<p><span class='red'>&nbsp;MySQL failure: %s&nbsp;\n<br /></span></p>", $db_con->error);
                                    ...
                                    $link1  = str_replace("%20", " ", $link);   //  make it readable
                                    $link1  = str_replace("%25", "%", $link);   //  make it readable
                                    ...
                                    if ($debug == '2') {
                                        if ($unreachable) {
                                            ...
                                            if ($command_line ==1 || $cl == 1) { //  no thumbnail ouput for command line operation
                                                ...
                                                $all = "    <table width='100%'>
                                                                <tr>
                                                                    ...
                                                                    <td> Title :&nbsp;&nbsp;$title_orig<br /><br />Size :&nbsp;&nbsp;$size[0] x $size[1] pixel<br /><br />Link : <a href='$link' target='rel' title='View link in new window'>".$link1."</a><br /><br /><span class='warnadmin'>No response, unreachable. $error</span><br /></td>
                                                                </tr>
                                                            </table>
                                                ";
                                            }
                                            ...
                                            if ($command_line ==1 || $cl == 1) { //  no thumbnail ouput for command line operation
                                                $all = "Title : $title_orig\r\n<br />Size : $size[0] x $size[1] pixel<br />\r\nLink : <a href='$link' target='rel' title='View link in new window'>".$link1."</a>\r\n<br />";
                                            } else {
                                                $output = str_replace($inst_dir, $install_dir, $output);
                                                $all = "<table width='100%'>
    <tr>
        <td class='cntr x4'><a href='$link' target='rel' title='View image in new window'><img src=\"$output\" border=1 alt=\"Thumbnail\" /></a></td>
        <td> Title :&nbsp;&nbsp;$title_orig<br /><br />Size :&nbsp;&nbsp;$size[0] x $size[1] pixel<br /><br />Link : <a href='$link' target='rel' title='View link in new window'>".$link1."</a><br /></td>
    </tr>
</table>
";
                                            }
            ...
            foreach ($all_media as $thisaudio) {
                ...
                if (($link = build_url($this_audio[2], $url, $select, $thisaudio[0], $handle, $store_file)) != '') { //  if valid URL was built
                    ...
                    if ($handle) {  //  really existing audio, or dead link only
                        ...
                        $link = $db_con->real_escape_string($link);
                        //      try to find already indexed audio stream with the same md5sum
                        $sql_query = "SELECT md5sum from ".$mysql_table_prefix."media where md5sum = '$new_md5'";
                        $result = $db_con->query($sql_query);
                        if ($debug && $db_con->errno) {
                            $err_row = __LINE__-2;
                        ...
                        if ($result->num_rows && $new_md5) {   //      enter only, if PHP function md5_file()  is allowed for remote files
                            ...
                            if (!$result->num_rows) {       //  if this audio is new
                                ...
                                mysqltest();
                                $sql_query = "INSERT into ".$mysql_table_prefix."media (link_id, link_addr, media_link, title, type, size_x, size_y, id3, md5sum, name, suffix) values ('$link_id', '$sql_url', '$sql_link', '$sql_title', '$element', '$size[0]', '$size[1]', '$sql_id3_string', '$new_md5', '$sql_my_name', '$suffix')";
                                $db_con->query ($sql_query);
                                if ($debug && $db_con->errno) {
                                    $err_row = __LINE__-2;
                                    printf("<p><span class='red'>&nbsp;MySQL failure: %s&nbsp;\n<br /></span></p>", $db_con->error);
                                ...
                                if ($debug == '2') {
                                    ...
                                    if ($index_id3 == 1 && $id3_string == '') {   //    unable to open the media link
                                        $output = "Title: $title_orig<br />Link : $link1";
                                        printActMedia($output);
                                        printUrlStatus("No response, ID3 tag unreachable.<br />", $command_line, $no_log);
                                    } else {
                                        $output = "Title:&nbsp;&nbsp;$title_orig<br />Link :<a href=\"$link\">$link1</a><br />";
                                        printActMedia($output);
                                    }
                                }
                    ...
                    if ($handle) {                      //  really existing video, or dead link only
                        ...
                        if ($result->num_rows && $new_md5) {   //      enter only, if PHP function md5_file()  is allowed for remote files
                            ...
                            if (!$result->num_rows) {                                       //  if this video is new
                                ...
                                mysqltest();
                                $sql_query = "INSERT into ".$mysql_table_prefix."media (link_id, link_addr, media_link, title, type, size_x, size_y, id3, md5sum, name, suffix) values ('$link_id', '$sql_url', '$sql_link', '$sql_title', '$element', '$size_x', '$size_y', '$sql_id3_string', '$new_md5', '$sql_my_name', '$suffix')";
                                $db_con->query ($sql_query);
                                if ($debug && $db_con->errno) {
                                    $err_row = __LINE__-2;
                                    printf("<p><span class='red'>&nbsp;MySQL failure: %s&nbsp;\n<br /></span></p>", $db_con->error);
