<?php
    ...
    (isset($_SESSION['admin']) && isset($_SESSION['admin_pw']) && crypt($_SESSION['admin'], $admin) == $admin && crypt($_SESSION['admin_pw'], $admin_pw) == $admin_pw ) || ($_SERVER['REMOTE_ADDR']=="")
    ) {
    } else {
        //  prepare token against XSRF attacks
        $token = generateToken();
        ...
        $fp = @fopen("$tmp_dir/$filename","w");    //  try to store the token into .../admin/tmp/token.txt
        if(!is_writeable("$tmp_dir/$filename")) {
            echo "  <br />
                        <p class='warnadmin cntr'><br />
