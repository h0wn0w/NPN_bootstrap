...
/********************************************
*
 * Sphider-plus version 3.2015e created 2015-09-24
 *
...
*/
    ...
    if (isset($url))            $url = cleaninput(substr(trim($url),0,1024));
...
";
    ...
    @fclose($fp);
    chmod("$include_dir/IDS/tmp/phpids_log.txt", 0777);    //  0777 required for command line operation
    if (array_key_exists ('HTTP_REFERER', $_SERVER) && !strpos($_SERVER['HTTP_REFERER'], "localhost")) {
        clearstatcache();
        $perms = substr(decoct(fileperms("$include_dir/IDS/tmp/phpids_log.txt")), 2);
        if ($perms != '0777') {
            echo "        <br />
        <p class='warnadmin cntr'><br />
        Attention: Sphider-plus is unable to set full write permission to the IDS log file.<br />
        Might cause problems for command line operation.<br />
        Modify the according server settings for PHP scripts.
        <br /><br /></p>
";
    ...
    if (array_key_exists ('HTTP_REFERER', $_SERVER) && !strpos($_SERVER['HTTP_REFERER'], "localhost")) {
        clearstatcache();
        $perms = substr(decoct(fileperms("$smap_dir/$testfile")), 2);
        if ($perms != '0777') {
            echo "        <br />
        <p class='warnadmin cntr'><br />
...
";
    ...
    if (array_key_exists ('HTTP_REFERER', $_SERVER) && !strpos($_SERVER['HTTP_REFERER'], "localhost")) {
        clearstatcache();
        $perms = substr(decoct(fileperms("$log_dir/$testfile")), 2);
        if ($perms != '0777') {
            echo "        <br />
        <p class='warnadmin cntr'><br />
...
";
    ...
    if (array_key_exists ('HTTP_REFERER', $_SERVER) && !strpos($_SERVER['HTTP_REFERER'], "localhost")) {
        clearstatcache();
        $perms = substr(decoct(fileperms("$thumb_folder/$testfile")), 2);
        if ($perms != '0777') {
            echo "        <br />
        <p class='warnadmin cntr'><br />
        ...
        echo "    <div class='submenu cntr'>|&nbsp;&nbsp;&nbsp;Database $dba_act&nbsp;&nbsp;with table prefix '$mysql_table_prefix'&nbsp;&nbsp;&nbsp;|</div>
                <div class='panel'>
                    ...
                    <table>
                        ...
                        <tr>
                            <td class='odrow bd'><a href='admin.php?f=clear_xml' class='options' title='Click to erase the folder'>Delete all files in 'XML' subfolder</a></td>
                            <td class='odrow'>| ".$xml_count." result files in folder</td>
                        </tr>
                        <tr>
                            <td class='evrow bd'><a href='admin.php?f=clear_smap' class='options' title='Click to erase the folder'>Delete all files in 'sitemaps' subfolder created during index procedures</a></td>
                            <td class='evrow'>| ".$sitemap_count." debug files in folder<br />&nbsp;</td>
                        </tr>
                        <tr>
