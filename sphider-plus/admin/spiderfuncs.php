...
//echo "\r\n\r\n<br>status Array final:<br><pre>";print_r($status);echo "</pre>\r\n";
    ...
    function get_links($file, $url, $can_leave_domain, $base, $media_links, $use_nofollow, $local_redir, $url_reloc, $charSet) {
        ...
        global $mainurl, $include_dir, $idna, $local, $index_rss, $index_alt, $lower_links, $debug, $ext, $js_reloc, $local_rfc;
...
//echo "\r\n\r\n<br>links array at the end of get_links():<br><pre>";print_r($links);echo "</pre>\r\n";
    ...
    function get_head_data($file, $url, $use_nofollow, $use_robot, $can_leave_domain, $type, $edititle, $edidescription) {
        global $db_con, $clear, $cano_leave, $only_links, $use_edidesc, $min_title, $min_desc;
...
//echo "\r\n\r\n<br>head_data array:<br><pre>";print_r($data);echo "</pre>\r\n";
    ...
    function clean_file($file, $url, $type, $charSet, $use_nofollow, $use_robot, $can_leave_domain, $edititle, $edidescription) {
        ...
        global $not_uls, $not_ullist, $not_pres, $not_prelist;
        ...
        if ($ignore_comment) {
            ...
            if (stripos(substr($file, 0, 100), "<!DOCTYPE")) {
        ...
        $first_body_pos = stripos($file, "<body");
        if ($first_body_pos) {
            $sec_body_pos = stripos($file, "<body", $first_body_pos+5);
            if ($sec_body_pos) {
...
//  <div id="currentSizeSystem" class="s-hidden">        EU    </div>
        ...
        //  kill remaining trash (all seen on www)
        ...
        $data['fulltext']       = trim($db_con->real_escape_string($fulltext));     //  will be used as text with highlighted keywords in result listing
        $data['content']        = trim($db_con->real_escape_string($file));         //  will be used to extract the keywords
        $data['title']          = trim($db_con->real_escape_string($title));        //  title tag from HTML head
        $data['description']    = trim($db_con->real_escape_string($description));  //  description tag from HTML head
        $data['keywords']       = trim($db_con->real_escape_string($keywords));     //  keywords tag from HTML head
        $data['host']           = $host;
        $data['path']           = $path;
...
echo $a->output();
            ...
            //  test for converter  filetotext
            $filename .= ".".$source_type."";
        }
echo "\r\n\r\n<br /> filename: '$filename'<br />\r\n";
            require_once "".$converter_dir."/filetotext.php";
...
*/
    ...
    function create_sitemap($site_id, $url) {
        global $db_con, $mysql_table_prefix, $inst_dir, $smap_dir, $smap_unique, $debug, $clear;
...
\\5</pre>",
    ...
    function index_url($url, $level, $site_id, $md5sum, $domain, $indexdate, $sessid, $can_leave_domain, $reindex, $use_nofollow, $cl, $use_robot, $use_pref, $url_inc, $url_not_inc, $num, $edititle, $edidescription) {
        ...
        global $docs, $only_docs, $only_links, $case_sensitive, $vowels, $noacc_el, $include_dir, $thumb_folder, $js_reloc, $server_char, $ignore_text;
        global $latin_ligatures, $phon_trans, $liga, $inst_dir, $install_dir, $admin_dir, $admin_url;
        ...
        if ($smp != 1 && $follow_sitemap == 1) {        //  enter here if we don't already know a valid sitemap and if admin settings allowed us to do so
            ...
            $log_file       = "./sitemaps/current_sitemap_".date("Y-m-d_H.i.s")."_".mt_rand().".xml"; //      destination for sitemap log-file
            $smap_found     = '';
            $indexed_map    = '';
            $map_cont       = '';
                        ...
                        if ($link_check == 0) {
                            ...
                            if (is_array($wordarray) && count($wordarray) >= $min_words_per_page) {
                                ...
                                if ($md5sum == '' || ($md5sum == '' && $url_status['relocate'])) {
                                    ...
                                    //$sql_query = "INSERT into ".$mysql_table_prefix."links (site_id, url, title, description, indexdate, size, md5sum, level, webshot) values ('$site_id', '$url', '$title', '$desc', curdate(), '$pageSize', '$newmd5sum', '$thislevel', '$shot')";
                                    $db_con->query ($sql_query);
                                    if ($debug && $db_con->errno) {
                                        $err_row = __LINE__-2;
                                        printf("<p><span class='red'>&nbsp;MySQL failure: %s&nbsp;\n<br /></span></p>", $db_con->error);
