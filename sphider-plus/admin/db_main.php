<?php
    ...
    if (isset($db1_set))
    $db1_set = substr(trim($db1_set),0,1);
    if (isset($send2))
    $send2 = cleaninput(substr(trim($send2),0,255));

    $del    = cleaninput(substr(trim($del),0,2));
    $sel    = cleaninput(substr(trim($sel),0,2));
    $db     = cleaninput(substr(trim($db),0,2));
    ...
    if($send2 == "Backup") {
        ...
        $flush = '';
        optimize($flush);	//	before backup, preventively repair and optimize current database
...
*/
    ...
    //      Enter here to restore any backup file into database and also restore thumbnails
    ...
    $database = $db_con -> real_escape_string($database);
    ...
    if ($tables) {
        ...
        echo " </table>
            <form name='optimize' id='optimize' method='get' action='admin.php'>
                <p class='tblhead'>
                <input type='hidden' name='f' value='database' />
                <input type='hidden' name='db' value='".$db."' />
