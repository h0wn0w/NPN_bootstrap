<?php
    ...
    if (isset($query)){             //  contains the query string
        $query = cleaninput(trim(substr($query,0,50)));
    }

    if (isset($submit)){            //  contains search mode (sites, links, keywords)
        $submit = urlencode($submit);
        //$submit = str_replace(chr(173), "", $submit);   //  will kill any &shy;
        //$submit = str_replace("&nbsp;&nbsp;", "", $submit);  //  kill duplicate &nbsp; blanks
        $submit = preg_replace("/[%C2%A0]+/", "", $submit);  //  will kill TABs
        $submit = trim(substr($submit,0,50));
    }
    ...
    echo "
            <br /><br />
            <a class='bkbtn' href='admin.php?f=2' title='Go back'>Back to Admin's Sites view</a>
            <br /><br />
        </body>
    </html>
                ";
