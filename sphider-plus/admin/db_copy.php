<?php
    ...
    if (isset($copy))
    $copy = substr(trim($copy),0,1);
    if (isset($db_source))
    $db_source = substr(trim($db_source),0,1);
    if (isset($db_dest))
    $db_dest = substr(trim($db_dest),0,1);
    ...
    if (isset($copy)) {
        ...
        $flush = '';
        optimize($flush);	//	before copy, preventively repair and optimize current database
