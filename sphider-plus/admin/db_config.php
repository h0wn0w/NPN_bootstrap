<?php
    ...
    if (!defined("_SECURE")) {
        ...
        if (isset($submit))
        $submit = cleaninput(substr(trim($submit),0,2));
        if (isset($_db_count))
        $_db_count = cleaninput(substr(trim($_db_count),0,2));

        if (isset($database1))
        $database1 = cleaninput(substr(trim($database1),0,255));
        if (isset($mysql_user1))
        $mysql_user1 = cleaninput(substr(trim($mysql_user1),0,255));
        if (isset($password1_hd))
        $password1_hd = cleaninput(substr(trim($password1_hd),0,255));
        if (isset($mysql_host1))
        $mysql_host1 = cleaninput(substr(trim($mysql_host1),0,255));
        if (isset($mysql_table_prefix1))
        $mysql_table_prefix1 = cleaninput(substr(trim($mysql_table_prefix1),0,255));

        if (isset($database2))
        $database2 = cleaninput(substr(trim($database2),0,255));
        if (isset($mysql_user2))
        $mysql_user2 = cleaninput(substr(trim($mysql_user2),0,255));
        if (isset($password2_hd))
        $password2_hd = cleaninput(substr(trim($password2_hd),0,255));
        if (isset($mysql_host2))
        $mysql_host2 = cleaninput(substr(trim($mysql_host2),0,255));
        if (isset($mysql_table_prefix2))
        $mysql_table_prefix2 = cleaninput(substr(trim($mysql_table_prefix2),0,255));

        if (isset($database3))
        $database3 = cleaninput(substr(trim($database3),0,255));
        if (isset($mysql_user3))
        $mysql_user3 = cleaninput(substr(trim($mysql_user3),0,255));
        if (isset($password3_hd))
        $password3_hd = cleaninput(substr(trim($password3_hd),0,255));
        if (isset($mysql_host3))
        $mysql_host3 = cleaninput(substr(trim($mysql_host3),0,255));
        if (isset($mysql_table_prefix3))
        $mysql_table_prefix3 = cleaninput(substr(trim($mysql_table_prefix3),0,255));

        if (isset($database4))
        $database4 = cleaninput(substr(trim($database4),0,255));
        if (isset($mysql_user4))
        $mysql_user4 = cleaninput(substr(trim($mysql_user4),0,255));
        if (isset($password4_hd))
        $password4_hd = cleaninput(substr(trim($password4_hd),0,255));
        if (isset($mysql_host4))
        $mysql_host4 = cleaninput(substr(trim($mysql_host4),0,255));
        if (isset($mysql_table_prefix4))
        $mysql_table_prefix4 = cleaninput(substr(trim($mysql_table_prefix4),0,255));

        if (isset($database5))
        $database5 = cleaninput(substr(trim($database5),0,255));
        if (isset($mysql_user5))
        $mysql_user5 = cleaninput(substr(trim($mysql_user5),0,255));
        if (isset($password5_hd))
        $password5_hd = cleaninput(substr(trim($password5_hd),0,255));
        if (isset($mysql_host5))
        $mysql_host5 = cleaninput(substr(trim($mysql_host5),0,255));
        if (isset($mysql_table_prefix5))
        $mysql_table_prefix5 = cleaninput(substr(trim($mysql_table_prefix5),0,255));
    ...
    function check_dbs($output, $mysql_table_prefix1, $mysql_table_prefix2, $mysql_table_prefix3, $mysql_table_prefix4, $mysql_table_prefix5) {
        global $debug, $db_con, $plus_nr, $db1_slv, $db2_slv, $db3_slv, $db4_slv, $db5_slv, $latest_field, $latest_table, $settings_dir ;
